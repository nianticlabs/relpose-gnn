...
from niantic.utils import pose_utils
...
class PoseNetTransCriterion(nn.Module):
    def __init__(self, t_loss_fn=nn.L1Loss(), q_loss_fn=nn.L1Loss(), sax=0,
                 saq=0, learn_beta=True):
        super(PoseNetTransCriterion, self).__init__()
        self.t_loss_fn = t_loss_fn
        self.q_loss_fn = q_loss_fn
        self.sax = nn.Parameter(torch.Tensor([sax]), requires_grad=learn_beta)
        self.saq = nn.Parameter(torch.Tensor([saq]), requires_grad=learn_beta)
    ...
    def forward(self, pred, targ):
        ...
        # print('sax, saq: ', self.sax, self.saq)
        return loss


class PoseNetRotCriterion(nn.Module):
    def __init__(self, t_loss_fn=nn.L1Loss(), q_loss_fn=nn.L1Loss(), sax=0,
                 saq=0, learn_beta=True):
        super(PoseNetRotCriterion, self).__init__()
        self.t_loss_fn = t_loss_fn
        self.q_loss_fn = q_loss_fn
        self.sax = nn.Parameter(torch.Tensor([sax]), requires_grad=learn_beta)
        self.saq = nn.Parameter(torch.Tensor([saq]), requires_grad=learn_beta)
    ...
    def forward(self, pred, targ):
        ...
        loss = torch.exp(-self.saq) * self.q_loss_fn(pred[..., 3:], targ[..., 3:]) + self.saq
        # print('sax, saq: ', self.sax, self.saq)
        return loss


class MapNetCriterion(nn.Module):
    def __init__(self, t_loss_fn=nn.L1Loss(), q_loss_fn=nn.L1Loss(), sax=-1.0,
                 saq=-4.0, srx=-1, srq=-4.0, learn_beta=True, learn_gamma=True):
        """
...
class AtLocCriterion(nn.Module):
    def __init__(self, t_loss_fn=nn.L1Loss(), q_loss_fn=nn.L1Loss(), sax=0.0, saq=0.0,
                 learn_beta=False):
...
class AtLocPlusCriterion(nn.Module):
    def __init__(self, t_loss_fn=nn.L1Loss(), q_loss_fn=nn.L1Loss(), sax=0.0, saq=0.0, srx=0.0,
                 srq=0.0, learn_beta=False, learn_gamma=False):
...
class PoseNetCriterion_original(nn.Module):
    def __init__(self, t_loss_fn=nn.MSELoss(), q_loss_fn=nn.MSELoss(), sax=0,
                 saq=0, learn_beta=True):
        super(PoseNetCriterion_original, self).__init__()
        self.t_loss_fn = t_loss_fn
        self.q_loss_fn = q_loss_fn
        self.sax = nn.Parameter(torch.Tensor([sax]), requires_grad=learn_beta)
        self.saq = nn.Parameter(torch.Tensor([saq]), requires_grad=learn_beta)
    ...
    def forward(self, pred, targ):
        ...
        :return:
        """
        targ[..., 3:] = F.normalize(targ[..., 3:], p=2, dim=1)
